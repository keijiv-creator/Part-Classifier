...
UNIQUE_HEADERS = [
    'Database', 'Line Status', 'Order Date', 'Sales Order No', 'So Status',
    'Quote No', 'Customer ID', 'Customer Name', 'SO line No',
    'National Part ID', 'Customer Part No', 'Part Description'
]

TOTALS_HEADERS = [
    'Order Date', 'Sales Order No', 'Quote No', 'Customer Name',
...
LANDMARK_HEADERS = [
    'NAT PART ID', 'CUST PART ID', 'FIRST ORDER DATE',
    'FIRST ORDER NO', 'QUOTE NO'
]


def process_raw_booking_to_natman(raw_file, output_file):
    print("  Processing raw Development Booking into Natman Bookings format...")
    wb_in = openpyxl.load_workbook(raw_file, read_only=True, data_only=True)
    ws_in = wb_in.active
    raw_rows = list(ws_in.iter_rows(values_only=True))
    wb_in.close()

    if not raw_rows:
        print("  ERROR: Raw booking file is empty")
        return None

    raw_headers = [str(h).strip() if h else '' for h in raw_rows[0]]
    hi = {h: i for i, h in enumerate(raw_headers)}
    data_rows = raw_rows[1:]

    main_data = []
    for row in data_rows:
        mapped = []
        for rh, mh in zip(RAW_BOOKING_HEADERS, MAIN_HEADERS):
            idx = hi.get(rh)
            val = row[idx] if idx is not None and idx < len(row) else None
            mapped.append(val)
        main_data.append(mapped)

    seen_unique = set()
    unique_data = []
    for row in main_data:
        nat_part_id = str(row[8]).strip() if row[8] is not None else ''
        so_no = str(row[2]).strip() if row[2] is not None else ''
        key = (nat_part_id, so_no)
        if key not in seen_unique:
            ...
            unique_row = [
                ...
                row[12],  # Line Status
                row[1],   # Order Date
                row[2],   # Sales Order No
                row[3],   # So Status
                row[4],   # Quote No
                row[5],   # Customer ID
                row[6],   # Customer Name
                row[7],   # SO line No
                row[8],   # National Part ID
                row[9],   # Customer Part No
                row[10],  # Part Description
            ]
    ...
    so_groups = defaultdict(lambda: {
        ...
        'customer_name': None, 'nat_part_id': None, 'cust_part_no': None,
        'order_qty': 0, 'total_amount': 0, 'shipped_qty': 0,
        'fulfill_qty': 0, 'pending_qty': 0, 'pending_amount': 0
    })
    for row in main_data:
        nat_part_id = str(row[8]).strip() if row[8] is not None else ''
        so_no = str(row[2]).strip() if row[2] is not None else ''
        key = (nat_part_id, so_no)
        g = so_groups[key]
        if g['order_date'] is None:
            g['order_date'] = row[1]
            g['so_no'] = row[2]
            g['quote_no'] = row[4]
            g['customer_name'] = row[6]
            g['nat_part_id'] = row[8]
            g['cust_part_no'] = row[9]
        def to_num(v):
            if v is None:
                return 0
            ...
            try:
                ...
                return 0
        g['order_qty'] += to_num(row[11])
        g['total_amount'] += to_num(row[14])
        g['shipped_qty'] += to_num(row[15])
        g['fulfill_qty'] += to_num(row[16])
        g['pending_qty'] += to_num(row[17])
        pa = row[18]
        if isinstance(pa, str) and pa.strip() == '-':
            pa = 0
        g['pending_amount'] += to_num(pa)

    for key in so_groups:
        g = so_groups[key]
        totals_data.append([
            g['order_date'], g['so_no'], g['quote_no'], g['customer_name'],
            g['nat_part_id'], g['cust_part_no'], g['order_qty'],
            g['total_amount'], g['shipped_qty'], g['fulfill_qty'],
            g['pending_qty'], g['pending_amount']
        ])

    landmark_map = {}
    for row in main_data:
        nat_part_id = str(row[8]).strip().upper() if row[8] is not None else ''
        cust_part = str(row[9]).strip() if row[9] is not None else ''
        order_date = row[1]
        so_no = row[2]
        quote_no = row[4]
        if not nat_part_id:
            continue
        lkey = (nat_part_id, cust_part)
        if lkey not in landmark_map:
            landmark_map[lkey] = {
                'nat_part_id': nat_part_id,
                'cust_part_id': cust_part,
                'first_order_date': order_date,
                'first_order_no': so_no,
                'quote_no': quote_no
            }
        else:
            existing_date = landmark_map[lkey]['first_order_date']
            if order_date is not None and existing_date is not None:
                try:
                    ...
                    pass
            elif order_date is not None and existing_date is None:
                landmark_map[lkey]['first_order_date'] = order_date
                landmark_map[lkey]['first_order_no'] = so_no
                landmark_map[lkey]['quote_no'] = quote_no

    landmark_data = []
    for lkey in sorted(landmark_map.keys()):
        lm = landmark_map[lkey]
        landmark_data.append([
            lm['nat_part_id'], lm['cust_part_id'],
            lm['first_order_date'], lm['first_order_no'], lm['quote_no']
        ])
    ...
    ws_unique.append(UNIQUE_HEADERS)
    ...
    for row in totals_data:
        ws_totals.append(row)

    ws_landmark = wb_out.create_sheet('LANDMARK')
    ws_landmark.append(LANDMARK_HEADERS)
    for row in landmark_data:
        ws_landmark.append(row)

    wb_out.save(output_file)
    wb_out.close()
    print(f"  Created Natman Bookings: {os.path.basename(output_file)}")
    print(f"    MAIN: {len(main_data)} rows")
    print(f"    UNIQUE: {len(unique_data)} rows")
    print(f"    TOTALS: {len(totals_data)} rows")
    print(f"    LANDMARK: {len(landmark_data)} rows")
    return output_file
...
def main(cli_args=None):
    ...
    try:
        wb_check = openpyxl.load_workbook(bookings_file, read_only=True)
        has_landmark = 'LANDMARK' in wb_check.sheetnames
        wb_check.close()
    except Exception:
        has_landmark = False

    if not has_landmark:
        print("  Detected raw Development Booking file (no LANDMARK sheet)")
        natman_output = os.path.join(NATMAN_EXTRACT, 'Natman_Bookings_generated.xlsx')
        result = process_raw_booking_to_natman(bookings_file, natman_output)
        if result:
            bookings_file = result
        else:
            print("  ERROR: Failed to process raw booking file")
            sys.exit(1)
